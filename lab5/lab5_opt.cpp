...
// #define POLY1_SIZE 9711
// #define POLY2_SIZE 8141

#define POLY1_SIZE 1000
#define POLY2_SIZE 1000
#define POLY_SEGMENT_SIZE 64
...
void segmentMultiplication(Polynomial &p1, Polynomial &p2, vector<atomic<int>> &result_coefficients, int p1_start) { 
    int pol1_size = p1.getDegree(), pol2_size = p2.getDegree();

    for (int d1 = p1_start; d1 <= pol1_size; ++d1)
    {
        for (int d2 = 0; d2 <= pol2_size; ++d2)
        {
            result_coefficients[d1 + d2].store(result_coefficients[d1 + d2] + p1.coefficients[d1] * p2.coefficients[d2]) ;
        }
    }
}
...
Polynomial polynomialMultiplicationMT(Polynomial &p1, Polynomial &p2) { 
    ...
    int pol1_size = p1.getDegree(), pol2_size = p2.getDegree();
    vector<atomic<int>> result_coefficients(max_degree);

    max_degree = max(pol1_size, pol2_size);

    {
        ThreadPool pool(THREAD_POOL_SIZE);

        for (int p1_start = 0; p1_start <= pol1_size; p1_start += 250) // max_degree / THREAD_POOL_SIZE)
            pool.enqueue([&]
                         { segmentMultiplication(ref(p1), ref(p2), ref(result_coefficients), p1_start); });
    }

    max_degree = p1.getDegree() + p2.getDegree() + 1;
    vector<int> final(max_degree);
    for (int i = 0; i < max_degree; ++i)
        final[i] = result_coefficients[i];
...
{
    try{
        Polynomial test1 = generatePolynomial(POLY1_SIZE);
        Polynomial test2 = generatePolynomial(POLY2_SIZE);
