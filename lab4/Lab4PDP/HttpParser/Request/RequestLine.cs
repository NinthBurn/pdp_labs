    ...
    public class RequestLine
    {
        ...
        private void ValidateRequestLine(string[] firstLine)
        {
            if (firstLine.Length != 3)
                throw new HttpParserException("Request Line is not in a valid format", "ValidateRequestLine", string.Join(" ", firstLine));
        }
        ...
        private void SetHttpMethod(string method)
        {
            ...
            if (!validHttpVerbs.Contains(method))
                throw new HttpParserException($"Not a valid HTTP Verb", "SetHttpMethod", method);
        ...
        private void SetUrl(string url)
        {
            if (!IsValidUri(url, out Uri _))
                throw new HttpParserException($"URL is not in a valid format", "SetUrl", url);
